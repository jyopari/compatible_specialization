...
class MultiLayerRoutedGPT(GPT):
    ...
    def _add_experts(self, args, router_config):
        ...
        for i, layer in enumerate(self.transformer.h):
            ...
            if i == len(self.transformer.h) - 1: #TODO !!!!!!!!!!!!!!!!!!!!! FIX XXX 
                for k in range(1, self.num_multi_layer_experts):
                    experts.update(
                        {
                            f"mlp_{j + k * len(args.router_ckpts)}": self.library.get_model(f"model_{j}_mlp_{i-k}")
                            for j in range(len(args.router_ckpts))
                        }   
                    )
            elif i == len(self.transformer.h) - 2: #TODO SUPER HACKY 
                    experts.update(
                        {
                            f"mlp_{j + 1 * len(args.router_ckpts)}": self.library.get_model(f"model_{j}_mlp_{i-1}")
                            for j in range(len(args.router_ckpts))
                        }   
                    )
                    experts.update(
                        {
                            f"mlp_{j + 2 * len(args.router_ckpts)}": self.library.get_model(f"model_{j}_mlp_{i-2}")
                            for j in range(len(args.router_ckpts))
                        }   
                    )
                    experts.update(
                        {
                            f"mlp_{j + 3 * len(args.router_ckpts)}": self.library.get_model(f"model_{j}_mlp_{i+1}")
                            for j in range(len(args.router_ckpts))
                        }   
                    )
            elif i == len(self.transformer.h) - 3: #TODO SUPER HACKY
                    experts.update(
                        {
                            f"mlp_{j + 1 * len(args.router_ckpts)}": self.library.get_model(f"model_{j}_mlp_{i-1}")
                            for j in range(len(args.router_ckpts))
                    ...
                    experts.update(
                        {
                            f"mlp_{j + 2 * len(args.router_ckpts)}": self.library.get_model(f"model_{j}_mlp_{i+1}")
                            for j in range(len(args.router_ckpts))
                        }   
                    )
                    experts.update(
                        {
                            f"mlp_{j + 3 * len(args.router_ckpts)}": self.library.get_model(f"model_{j}_mlp_{i+2}")
                            for j in range(len(args.router_ckpts))
                        }   
                    )
            else:
